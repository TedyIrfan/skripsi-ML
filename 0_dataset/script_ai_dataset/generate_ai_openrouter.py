"""
Generate AI Text dari OpenRouter API
Target: 100 data (50 casual + 50 formal) dalam 5 batch
...
def generate_batch(style, batch_num):
    ...
    for attempt in range(max_retry):
        try:
            ...
            # Get content - check for reasoning models (GLM 4.5 Air puts content in reasoning)
            message = result["choices"][0]["message"]
            raw_text = message.get("content", "")

            # If content is empty but there's reasoning, use reasoning
            if not raw_text or len(raw_text.strip()) == 0:
                if "reasoning" in message and message["reasoning"]:
                    raw_text = message["reasoning"]
                    print(f"  Using reasoning field instead of content")
...
def main():
    ...
    print("\n" + "="*60)
    print("OPENROUTER AI TEXT GENERATOR")
    print(f"Model: {MODEL} (Hermes 3 405B)")
    ...
    for batch in range(1, 6):
        ...
        for t in texts:
            all_data.append({
                'text': t['text'],
                'label': 'AI',
                'model': 'OpenRouter-Hermes-3-405B',
                'style': 'Casual',
                'topic': t['topic'],
        ...
        time.sleep(2)  # Delay antar batch
    ...
    if all_data:
        ...
        # Save main file
        filename = f"data_ai_openrouter_{len(all_data)}.csv"
